...
export function getBuildById(id: number) {
    const build = db
        ...
        .get(id) as Build | null;
    return build;
}

export function getBuildByUuid(uuid: string) {
    const build = db
        .query("SELECT * FROM builds WHERE uuid = ?")
        .get(uuid) as Build | null;
    return build;
}

export function addBuild(build: AddBuild) {
    db.prepare(
        "INSERT INTO builds (uuid, name, build_path, git_repository) VALUES (?,?,?,?)",
    ).values(
        build.uuid ?? Uuid(),
        build.name,
        build.build_path,
...
export function updateBuild(build: Build) {
    db.prepare(
        `
        UPDATE builds 
        SET output = ? 
